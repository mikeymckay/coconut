  ...
  ReportView.prototype.weeklyReports = function(options) {
    ...
    return Reports.aggregateWeeklyReportsAndFacilityCases({
      ...
      success: function(results) {
        $("#reportContents").html("<style> td.number{ text-align: center; vertical-align: middle; } </style> <br/> <br/> <h1> Weekly Reports aggregated by <select style='height:50px;font-size:125%' id='aggregationPeriod'> " + (_("Year,Month,Week".split(",")).map((function(_this) {
          return function(aggregationPeriod) {
            return "<option " + (aggregationPeriod === _this.aggregationPeriod ? "selected='true'" : '') + "> " + aggregationPeriod + " </option>";
          };
        })(this)).join("")) + " </select> and <select style='height:50px;font-size:125%' id='aggregationArea'> " + (_("Zone,District,Facility".split(",")).map((function(_this) {
          return function(aggregationArea) {
            return "<option " + (aggregationArea === _this.aggregationArea ? "selected='true'" : '') + "> " + aggregationArea + " </option>";
          };
        })(this)).join("")) + " </select> </h1> <br/> <table class='tablesorter' id='weeklyReports'> <thead> <th>" + this.aggregationPeriod + "</th> <th>" + this.aggregationArea + "</th> " + (_.map(results.fields, function(field) {
          return "<th>" + field + "</th>";
          ...
          return _(aggregationAreas).map(function(data, aggregationArea) {
            ...
            return "<tr> <td>" + aggregationPeriod + "</td> <td>" + aggregationArea + "</td> " + (_.map(results.fields, function(field) {
              return "<td>" + (data[field] || 0) + "</td>";
            }).join("")) + " <td> " + (data["Mal POS < 5"] + data["Mal POS >= 5"]) + " </td> " + (percentElement = function(number) {
