...
ReportView = (function(_super) {
  ...
  ReportView.prototype.update = function() {
    ...
    return this.render(reportOptions);
  };
  ...
  ReportView.prototype.render = function(options) {
    ...
    this.aggregationPeriod = options.aggregationPeriod || "Month";
    this.aggregationArea = options.aggregationArea || "Zone";
    ...
    renderTable = _.after(optionsArray.length, (function(_this) {
      return function() {
        ...
        $("#alerts").html("<h2>Data Summary</h2> <table id='alertsTable' class='tablesorter'> <tbody> " + (index = 0, _(results[0]).map(function(firstResult) {
          var dataPoints, element, period, sum;
          return "<tr class='" + (index % 2 === 0 ? "odd" : "even") + "'> <td>" + firstResult.title + "</td> " + (period = results.length, sum = 0, dataPoints = 0, element = _.map(results, function(result) {
            if ((dataPoints + 1) !== results.length) {
              dataPoints += 1;
              sum += parseInt(dataValue(result[index]));
            }
            return "<td class='period-" + (period -= 1) + " trend'></td> <td class='period-" + period + " data'>" + (renderDataElement(result[index])) + "</td> " + (period === 0 ? "<td class='average-for-previous-periods'>" + (Math.round(sum / dataPoints)) + "</td>" : "");
  ...
  ReportView.prototype.analysis = function(aggregationLevel) {
    ...
    return reports.casesAggregatedForAnalysis({
      ...
      success: (function(_this) {
        return function(data) {
          var headings;
          headings = [aggregationLevel, "Cases", "Complete household visit", "%", "Missing USSD Notification", "Missing Case Notification", "Complete facility visit", "Without complete facility visit (but with case notification)", "%", "Without complete facility visit within 24 hours", "%", "Without complete household visit (but with complete facility visit)", "%", "Without complete household visit within 48 hours", "%"];
          ...
          $("#analysis").append(_this.createTable(headings, "" + (_.map(data.followups, function(values, location) {
            ...
            return "<tr> <td>" + location + "</td> <td>" + (_this.createDisaggregatableCaseGroup(values.allCases)) + "</td> <td>" + (_this.createDisaggregatableCaseGroup(values.casesWithCompleteHouseholdVisit)) + "</td> <td>" + (_this.formattedPercent(values.casesWithCompleteHouseholdVisit.length / values.allCases.length)) + "</td> <td class='missingUSSD details'>" + (_this.createDisaggregatableCaseGroup(values.missingUssdNotification)) + "</td> <td>" + (_this.createDisaggregatableCaseGroup(values.missingCaseNotification)) + "</td> <td class='details'>" + (_this.createDisaggregatableCaseGroup(values.casesWithCompleteFacilityVisit)) + "</td> " + (withoutcompletefacilityvisitbutwithcasenotification = _.difference(values.casesWithoutCompleteFacilityVisit, values.missingCaseNotification), "") + " <td>" + (_this.createDisaggregatableCaseGroup(withoutcompletefacilityvisitbutwithcasenotification)) + "</td> <td>" + (_this.formattedPercent(withoutcompletefacilityvisitbutwithcasenotification.length / values.allCases.length)) + "</td> <td>" + (_this.createDisaggregatableCaseGroup(values.noFacilityFollowupWithin24Hours)) + "</td> <td>" + (_this.formattedPercent(values.noFacilityFollowupWithin24Hours.length / values.allCases.length)) + "</td> " + (withoutcompletehouseholdvisitbutwithcompletefacility = _.difference(values.casesWithoutCompleteHouseholdVisit, values.casesWithCompleteFacilityVisit), "") + " <td>" + (_this.createDisaggregatableCaseGroup(withoutcompletehouseholdvisitbutwithcompletefacility)) + "</td> <td>" + (_this.formattedPercent(withoutcompletehouseholdvisitbutwithcompletefacility.length / values.allCases.length)) + "</td> <td>" + (_this.createDisaggregatableCaseGroup(values.noHouseholdFollowupWithin48Hours)) + "</td> <td>" + (_this.formattedPercent(values.noHouseholdFollowupWithin48Hours.length / values.allCases.length)) + "</td> </tr>";
          }).join("")), "cases-followed-up"));
          _(["Complete facility visit", "Missing USSD Notification"]).each(function(column) {
  ...
  ReportView.prototype.createCasesLinks = function(cases) {
    return _.map(cases, (function(_this) {
      return function(malariaCase) {
        return _this.createCaseLink({
          caseID: malariaCase.caseID || malariaCase
        });
      };
    })(this)).join("");
  };
  ...
  ReportView.prototype.createDisaggregatableCaseGroupWithLength = function(cases) {
    var text;
    text = cases ? cases.length : "-";
    return this.createDisaggregatableCaseGroup(cases, text);
  };
  ...
  ReportView.prototype.weeklyReports = function(options) {
    $("#row-region").hide();
    return Reports.aggregateWeeklyReportsAndFacilityCases({
      startDate: this.startDate,
      endDate: this.endDate,
      aggregationArea: this.aggregationArea,
      aggregationPeriod: this.aggregationPeriod,
      success: (function(_this) {
        return function(results) {
          $("#reportContents").html("<style> td.number{ text-align: center; vertical-align: middle; } </style> <br/> <br/> <h1> Weekly Reports aggregated by <select style='height:50px;font-size:125%' id='aggregationPeriod'> " + (_("Year,Month,Week".split(",")).map(function(aggregationPeriod) {
            return "<option " + (aggregationPeriod === _this.aggregationPeriod ? "selected='true'" : '') + "> " + aggregationPeriod + " </option>";
          }).join("")) + " </select> and <select style='height:50px;font-size:125%' id='aggregationArea'> " + (_("Zone,District,Facility".split(",")).map(function(aggregationArea) {
            return "<option " + (aggregationArea === _this.aggregationArea ? "selected='true'" : '') + "> " + aggregationArea + " </option>";
          }).join("")) + " </select> </h1> <br/> <table class='tablesorter' id='weeklyReports'> <thead> <th>" + _this.aggregationPeriod + "</th> <th>" + _this.aggregationArea + "</th> " + (_.map(results.fields, function(field) {
            return "<th>" + field + "</th>";
          }).join("")) + " <th>Weekly Reports Positive Cases</th> <th><5 Test Rate</th> <th><5 POS Rate</th> <th>=>5 Test Rate</th> <th>>=5 POS Rate</th> </thead> <tbody> " + (_(results.data).map(function(aggregationAreas, aggregationPeriod) {
            return _(aggregationAreas).map(function(data, aggregationArea) {
              var percentElement, total;
              if (aggregationArea === "Unknown") {
                return;
              }
              return "<tr> <td>" + aggregationPeriod + "</td> <td>" + aggregationArea + "</td> " + (_.map(results.fields, function(field) {
                if (field === "Facility Followed-Up Positive Cases") {
                  return "<td>" + (_this.createDisaggregatableCaseGroupWithLength(data[field])) + "</td>";
                } else {
                  return "<td>" + (data[field] != null ? data[field] : "-") + "</td>";
                }
              }).join("")) + " <td> " + (total = data["Mal POS < 5"] + data["Mal POS >= 5"], Number.isNaN(total) ? '-' : total) + " </td> " + (percentElement = function(number) {
                if (Number.isNaN(number)) {
                  return "<td>-</td>";
                } else {
                  return "<td>" + (Math.round(number * 100)) + "%</td>";
                }
              }, "") + " " + (percentElement((data["Mal POS < 5"] + data["Mal NEG < 5"]) / data["All OPD < 5"])) + " " + (percentElement(data["Mal POS < 5"] / (data["Mal NEG < 5"] + data["Mal POS < 5"]))) + " " + (percentElement((data["Mal POS >= 5"] + data["Mal NEG >= 5"]) / data["All OPD >= 5"])) + " " + (percentElement(data["Mal POS >= 5"] / (data["Mal NEG >= 5"] + data["Mal POS >= 5"]))) + " </tr>";
            }).join("");
          }).join("")) + " </tbody> </table>");
          return $("#weeklyReports").dataTable({
